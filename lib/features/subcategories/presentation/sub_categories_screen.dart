...
//     final receive_id = ModalRoute.of(context)?.settings.arguments as int;
...
//                               if (id != null) {
//                                 detailsCubit.getCategoryDetails(id);
//                               }
//                             },
...
class SubCategoriesScreen extends StatefulWidget {
  ...
  // const SubCategoriesScreen({super.key, this.initialCategoryId});
...
class _SubCategoriesScreenState extends State<SubCategoriesScreen> {
  ...
  @override
  Widget build(BuildContext context) {
 final initialCategoryId = ModalRoute.of(context)?.settings.arguments as int?;
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (_) => getIt<CategoriesViewModel>()..getCategories(),
        ...
        body: BlocBuilder<CategoriesViewModel, CategoryState>(
          builder: (context, state) {
            ...
            if (state is CategorySuccess) {
              ...
              final selectedIndex = categoriesCubit.selectedIndex.clamp(
                0, 
                categories.length - 1
              );
              final selectedCategory = categories[selectedIndex];
